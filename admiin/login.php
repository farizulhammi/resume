...
if (isset($_POST['submit'])) {
  $user = protek($_POST['username']);
  $pass = protek($_POST['password']);
  $ip = get_ip();

  $qry = mysqli_query($conn,"SELECT * FROM user WHERE username='$user' ");
...
if ($cek == 1) {
    ...
    if (password_verify($pass, $data['password'])) {
      ...
      mysqli_query($conn, "INSERT INTO log VALUES ('','$user', '$ip', '$ymd $his' )");
      header("Location: $base_admin");

    }else {
      $respon =  '<div class="alert alert-danger alert-dismissible fade show">
      <button type="button" class="close" data-dismiss="alert">&times;</button><b>Gagal</b> Password Atau Username Yang Anda Masukan Salah
      </div>';
    }
  }else {
    $respon =  '<div class="alert alert-danger alert-dismissible fade show">
...
<html>
  <head>
    ...
    <title>Bubbly Admin - Login</title>
      ...
      <div class="container">
        <div class="row align-items-center py-5">
          ...
          <div class="col-lg-5 px-lg-4">
           ...
           <?= $respon; ?>
            <form id="loginForm" action="<?= $_SERVER['PHP_SELF'] ?>" method="POST" class="mt-4">
              ...
              <input type="submit" name="submit" class="btn btn-primary shadow px-5" value="Log in">
            </form>
          </div>
        </div>
